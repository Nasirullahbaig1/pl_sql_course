...
/
CREATE OR REPLACE PACKAGE PKG_EMP_UTIL_2 AS
--FUNCTION DEF
    FUNCTION FUN_DEP_SAL_EX_HIGH (P_IN_DEPT_ID IN DEPARTMENTS.DEPARTMENT_ID%TYPE) RETURN NUMBER;
    
    
END PKG_EMP_UTIL_2;
/
CREATE OR REPLACE PACKAGE BODY PKG_EMP_UTIL_2 AS
--FUNCTION BODY
    FUNCTION FUN_DEP_SAL_EX_HIGH (P_IN_DEPT_ID IN DEPARTMENTS.DEPARTMENT_ID%TYPE) RETURN NUMBER
    IS 
        V_SUM_SALARY NUMBER:=0;
    BEGIN
        SELECT SUM(SALARY) INTO V_SUM_SALARY FROM EMPLOYEES WHERE DEPARTMENT_ID = P_IN_DEPT_ID
        AND EMPLOYEE_ID <> (SELECT EMPLOYEE_ID FROM EMPLOYEES WHERE DEPARTMENT_ID = P_IN_DEPT_ID ORDER BY SALARY DESC FETCH FIRST ROW ONLY);
    RETURN V_SUM_SALARY;
    END FUN_DEP_SAL_EX_HIGH;
    
--PRIVATE PROCEDURE
    PROCEDURE P_VALIDATE_NEW_EMP_DATA (
            P_IN_FISRT_NAME IN VARCHAR2,
            P_IN_LAST_NAME  IN VARCHAR2,
            P_IN_DEPT_NAME  IN VARCHAR2,
            P_IN_SALARY     IN NUMBER,
            P_OUT_VALID     OUT VARCHAR2) IS
    BEGIN
        IF LENGHT(P_IN_FISRT_NAME) >20 OR LENGHT(P_IN_LAST_NAME) >25 
        OR NOT EXISTS (SELECT 1 FROM DEPARTMENTS WHERE UPPER(DEPARTMENT_NAME) = UPPER(P_IN_DEPT_NAME)) 
        OR EXISTS (SELECT 1 FROM EMPLOYEES WHERE UPPER(LAST_NAME) = UPPER(P_IN_LAST_NAME))
        THEN
            P_OUT_VALID := 'N';
        END IF;
    END P_VALIDATE_NEW_EMP_DATA;
        
/

SELECT PKG_EMP_UTIL_2.FUN_DEP_SAL_EX_HIGH(1) FROM DUAL;
SELECT * FROM EMPLOYEES;
